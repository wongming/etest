...
ConfHelpInfo = """\
The tool's default driver path is '.stest/driver'.
Default email template path is '.stest/EmailTemplate.html'.
You can create '.stest/stest.conf' for more configurations of stest.
Examples:
[base]
DriverPath = .stest/driver
EmailTemplatePath = .stest/EmailTemplate.html
TestMethodPrefix = tc_
[notice]
MailHost = smtp.xxx.com
MailUser = username
MailFrom = username@xxx.com
MailPwd = userpwd
"""
...
class TestProgram(object):
    ...
    def parseArgs(self, argv):
        ...
        import getopt
        long_opts = ["help", "version","case=", "plan=", "driver", "config"]
        short_opts= "hHvc:p:d"
        if len(argv)==0:
            self.usageExit()
        try:
